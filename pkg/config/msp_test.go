...
func TestUpdateMSP(t *testing.T) {
	...
	// TODO for now I've just set these all to a basic, yet valid, ca certificate
	// (in terms of the attributes)
	cert := x509.Certificate{
		KeyUsage: x509.KeyUsageKeyEncipherment | x509.KeyUsageDigitalSignature | x509.KeyUsageCertSign,
		IsCA:     true,
	}
	org1MSP.RootCerts = []x509.Certificate{cert}
	org1MSP.IntermediateCerts = []x509.Certificate{cert}
	org1MSP.TLSRootCerts = []x509.Certificate{cert}
	org1MSP.TLSIntermediateCerts = []x509.Certificate{cert}
...
func TestUpdateMSPFailure(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedErr: "invalid root cert: SerialNumber 7: KeyUsage must be x509.KeyUsageCertSign",
		},
		{
			...
			expectedErr: "invalid root cert: SerialNumber 7: must be a CA certificate",
		},
		{
			// TODO we fail here because the dummyCert doesn't have the attributes we need
			// for it to be used as a CA cert
			...
			expectedErr: "invalid intermediate cert: SerialNumber 7: KeyUsage must be x509.KeyUsageCertSign",
		},
	}
